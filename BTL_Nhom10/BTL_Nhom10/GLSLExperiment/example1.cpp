...
void shaderSetup(void)
{
	...
	vec4 color = ConvertRGBtoVec4(151, 255, 255);
	glClearColor(color.x, color.y, color.z, 1);        /* Thiết lập màu trắng là màu xóa màn hình*/
}
...
void view_projection() {
	...
	//camera
	vec4 eye =  Translate(xEye, yEye, 0 * xEye * tan(gocQuayNgangCam)) * vec4(0, 0.2, -6, 1);
	float eye_trucY = abs(eye.z);
	///cout << "eye_Y"<<eye_trucY;
	//RotateY(-gocQuayNgangCam)*
	vec4 at = Translate(xEye, yEye, 0) *Translate(sin(gocQuayNgangCam) * eye_trucY, 0 + tanf(gocQuayDocCam) * eye_trucY, cos(gocQuayNgangCam) * eye_trucY)* vec4(0, 0.2, -1, 1);
	at.w = 1;
...
void XeTenLua() {
	...
	setMau(215, 215, 215, 270);
	m = Translate(1 - 0.16, 0.2, 0) * RotateZ(20) * Scale(0.1, 0.25, 0.5);
	VeHinhHop(m, model_xeTenLua);
...
void XeTenLuaSieuVip() {
	model_xeTenLua = RotateY(ThetaXeTenLua[0]) * Translate(xXeTenLua, 0.4, 0);
	XeTenLua();
...
mat4 model_may;
void may() {
	setMau(248, 248, 245, 100, vec4(0, -10, 0, 1));
	mat4 m = Translate(0, 20, 15) * Scale(3, 0.1, 2);
	VeHinhHop(m, model_may);
	m = Translate(0.75 + 1.5, 20, 15 + 0.5) * Scale(1.5, 0.1, 1);
	VeHinhHop(m, model_may);
	...
	m = Translate(4, 0, 4) * Translate(0, 20, 15) * Scale(3, 0.1, 2);
	VeHinhHop(m, model_may);
	m = Translate(4, 0, 4) * Translate(0.75 + 1.5, 20, 15 + 0.5) * Scale(1.5, 0.1, 1);
	VeHinhHop(m, model_may);
	m = Translate(5, 20, 18) * Scale(3, 0.1, 2);
	VeHinhHop(m, model_may);
	m = Translate(0.75 + 1.5 + 5, 20, 15 + 0.5 + 3) * Scale(1.5, 0.1, 1);
...
float xmay = 0;
// CỦA LINH NGÂN	
mat4  model_truck1, model_truck, model_xetai_cuasau;
GLfloat a = 0, quaycanhphai = 0, quaycanhtrai = 0, quaycuasau1 = 0, quaycuasau2 = 0;
void xetai_canhxe1() {
	mat4 instance = Scale(0.15, 0.15, 0.02);
	glUniformMatrix4fv(model_loc, 1, GL_TRUE, model * model_truck * model_truck1 * instance);
	glDrawArrays(GL_TRIANGLES, 0, NumPoints);
}
void xetai_canhxe2(GLfloat x, GLfloat y, GLfloat z) {
	mat4 instance = Translate(x, y, z) * Scale(0.03, 0.15, 0.02);
	glUniformMatrix4fv(model_loc, 1, GL_TRUE, model * model_truck * model_truck1 * instance);
	glDrawArrays(GL_TRIANGLES, 0, NumPoints);
}
void xetai_canhxe3() {
	mat4 instance = Translate(0, 0.21, 0) * Scale(0.15, 0.03, 0.02);
	glUniformMatrix4fv(model_loc, 1, GL_TRUE, model * model_truck * model_truck1 * instance);
	glDrawArrays(GL_TRIANGLES, 0, NumPoints);
}
void xetai_taycamt() {
	vec4 l = vec4(0, 0, 0, 1);
	setMau(l);
	mat4 light = Translate(0.04, 0.02, 0.02) * Scale(0.02, 0.02, 0.01);
	glUniformMatrix4fv(model_loc, 1, GL_TRUE, model * model_truck * model_truck1 * light);
	glDrawArrays(GL_TRIANGLES, 0, NumPoints);
}
void xetai_taycamp() {
	vec4 l = vec4(0, 0, 0, 1);
	setMau(l);
	mat4 light2 = Translate(0.04, 0.02, -0.02) * Scale(0.01, 0.02, 0.01);
	glUniformMatrix4fv(model_loc, 1, GL_TRUE, model * model_truck * model_truck1 * light2);
	glDrawArrays(GL_TRIANGLES, 0, NumPoints);
}
mat4 model_xetai_kinh;
void xetai_kinhphai() {
	vec4 l = vec4(0.75, 0.75, 0.75, 0.5);
	setMau(l);
	mat4 instance = Translate(0, 0.13, 0) * Scale(0.1, 0.13, 0.01);
	glUniformMatrix4fv(model_loc, 1, GL_TRUE, model * model_truck * model_truck1 * model_xetai_kinh * instance);
	glDrawArrays(GL_TRIANGLES, 0, NumPoints);
}
GLfloat t = 0, q = 0, e = 0;
void xetai_canhxephai1() {
	xetai_canhxe1();
	xetai_canhxe2(-0.065, 0.15, 0);
	xetai_canhxe2(0.065, 0.15, 0);
	xetai_canhxe3();
	model_xetai_kinh = Translate(t, q, e);
	xetai_kinhphai();
}
void xetai_canhxephai() {
	vec4 l = vec4(0, 0.4, 0, 1);
	setMau(l);
	model_truck1 = RotateY(quaycanhphai) * Translate(0.075, 0, 0);
	xetai_canhxephai1();
	xetai_taycamp();

}
void xetai_canhxetrai() {
	vec4 l = vec4(0, 0.4, 0, 1);
	setMau(l);
	model_truck1 = RotateY(quaycanhtrai) * Translate(0.075, 0, 0);
	xetai_canhxephai1();
	xetai_taycamt();

}
void xetai_mattren1(GLfloat x, GLfloat y, GLfloat z) {
	mat4 instance = Translate(x, y, z) * Scale(0.15, 0.15, 0.02);
	glUniformMatrix4fv(model_loc, 1, GL_TRUE, model * model_truck * instance);
	glDrawArrays(GL_TRIANGLES, 0, NumPoints);
}
void xetai_mattren2(GLfloat x, GLfloat y, GLfloat z) {
	mat4 instance = Translate(x, y, z) * Scale(0.15, 0.02, 0.3);
	glUniformMatrix4fv(model_loc, 1, GL_TRUE, model * model_truck * instance);
	glDrawArrays(GL_TRIANGLES, 0, NumPoints);
}
void xetai_mattren3(GLfloat x, GLfloat y, GLfloat z) {
	mat4 instance = Translate(x, y, z) * Scale(0.02, 0.17, 0.3);
	glUniformMatrix4fv(model_loc, 1, GL_TRUE, model * model_truck * instance);
	glDrawArrays(GL_TRIANGLES, 0, NumPoints);
}

void xetai_mattren4() {
	mat4 instance = Translate(0.095, 0.205, 0) * Scale(0.02, 0.02, 0.26);
	glUniformMatrix4fv(model_loc, 1, GL_TRUE, model * model_truck * instance);
	glDrawArrays(GL_TRIANGLES, 0, NumPoints);
}
void xetai_mattren5() {
	mat4 instance = Translate(0.095 + 0.065, 0.215, 0) * Scale(0.15, 0.02, 0.26);
	glUniformMatrix4fv(model_loc, 1, GL_TRUE, model * model_truck * instance);
	glDrawArrays(GL_TRIANGLES, 0, NumPoints);
}
void xetai_denxe() {
	// Màu của đèn xe
	vec4 l = vec4(1, 0.5, 0, 1);
	setMau(l);
	// Vẽ đèn xe trước mặt trên của xe
	mat4 light = Translate(-0.1, 0, 0.08) * Scale(0.01, 0.02, 0.02);
	glUniformMatrix4fv(model_loc, 1, GL_TRUE, model * model_truck * light);
	glDrawArrays(GL_TRIANGLES, 0, NumPoints);
	mat4 light2 = Translate(-0.1, 0, -0.08) * Scale(0.01, 0.02, 0.02);
	glUniformMatrix4fv(model_loc, 1, GL_TRUE, model * model_truck * light2);
	glDrawArrays(GL_TRIANGLES, 0, NumPoints);
}
void xetai_kinhtren() {
	vec4 l = vec4(0.75, 0.75, 0.75, 1);
	setMau(l);
	mat4 instance = Translate(0.1, 0.13, 0) * Scale(0.01, 0.13, 0.26);
	glUniformMatrix4fv(model_loc, 1, GL_TRUE, model * model_truck * instance);
	glDrawArrays(GL_TRIANGLES, 0, NumPoints);
}
void xetai_mattren() {
	vec4 l = vec4(0, 0.4, 0, 1);
	setMau(l);
	xetai_mattren1(0, 0, 0.14);
	xetai_mattren1(0, 0, -0.14);
	xetai_mattren2(0, 0.085, 0);
	xetai_mattren3(-0.085, 0.01, 0);
	xetai_mattren3(0.085, 0.01, 0);
	xetai_mattren4();
	xetai_mattren5();
	xetai_kinhtren();
...
void xetai_matduoi1() {
	//	mat4 instance = Translate(0.3 - 0.05, 0.075, 0) * Scale(0.02, 0.3, 0.3);
	mat4 instance = Scale(0.02, 0.3, 0.3);
	glUniformMatrix4fv(model_loc, 1, GL_TRUE, model * model_truck * instance);
	glDrawArrays(GL_TRIANGLES, 0, NumPoints);
}
void xetai_matduoi2(GLfloat x, GLfloat y, GLfloat z) {
	mat4 instance = Translate(x, y, z) * Scale(0.3, 0.3, 0.02);
	glUniformMatrix4fv(model_loc, 1, GL_TRUE, model * model_truck * instance);
	glDrawArrays(GL_TRIANGLES, 0, NumPoints);
}
void xetai_matduoi3() {
	mat4 instance = Translate(0.15, 0.15, 0) * Scale(0.3, 0.02, 0.3);
	glUniformMatrix4fv(model_loc, 1, GL_TRUE, model * model_truck * instance);
	glDrawArrays(GL_TRIANGLES, 0, NumPoints);
}
void xetai_matduoi4() {
	mat4 instance = Translate(-0.02, -0.16, 0) * Scale(0.64, 0.02, 0.3);
	glUniformMatrix4fv(model_loc, 1, GL_TRUE, model * model_truck * instance);
	glDrawArrays(GL_TRIANGLES, 0, NumPoints);
}
void xetai_matduoi() {
	vec4 l = vec4(0, 0.5, 0, 1);
	setMau(l);
	xetai_matduoi1();
	xetai_matduoi2(0.15, 0, 0.15);
	xetai_matduoi2(0.15, 0, -0.15);
	xetai_matduoi3();
	xetai_matduoi4();
}
void xetai_cuasau1() {
	mat4 instance = Scale(0.02, 0.305, 0.15);
	glUniformMatrix4fv(model_loc, 1, GL_TRUE, model * model_truck * model_xetai_cuasau * instance);
	glDrawArrays(GL_TRIANGLES, 0, NumPoints);
}
void xetai_cuasau2() {
	mat4 instance = Scale(0.02, 0.305, 0.15);
...
void xetai_cuasau() {
	vec4 l = vec4(0, 0.5, 0, 1);
	setMau(l);
	model_xetai_cuasau = Translate(0, 0, 0.15) * RotateY(quaycuasau1) * Translate(0, 0, -0.075);
	xetai_cuasau1();
	model_xetai_cuasau = Translate(0, 0, -0.15) * RotateY(quaycuasau2) * Translate(0, 0, 0.075);
	xetai_cuasau2();
}
GLfloat xetai_quaybanhxe = 0;
mat4 model_xetai_banhxe;
void xetai_duongtron(GLfloat k) {
	mat4 instance = RotateZ(k) * Scale(0.15, 0.01, 0.05);
	glUniformMatrix4fv(model_loc, 1, GL_TRUE, model * model_truck * model_xetai_banhxe * instance);
	glDrawArrays(GL_TRIANGLES, 0, NumPoints);
}
void xetai_banhxe1() {
	for (int i = 1; i <= 180; i++) {
...
void xetai_banhxe() {
	vec4 l = vec4(0, 0, 0, 1);
	setMau(l);
	model_xetai_banhxe = Translate(-0.09, -0.105, -0.17) * RotateZ(xetai_quaybanhxe);
	xetai_banhxe1();
	model_xetai_banhxe = Translate(-0.09, -0.105, 0.17) * RotateZ(xetai_quaybanhxe);
	xetai_banhxe2();
	model_xetai_banhxe = Translate(0.3, -0.105, -0.17) * RotateZ(xetai_quaybanhxe);
	xetai_banhxe3();
	model_xetai_banhxe = Translate(0.3, -0.105, 0.17) * RotateZ(xetai_quaybanhxe);
	xetai_banhxe4();
...
GLfloat len;
void Truck() {
	model_truck = RotateY(a) * Translate(len, 0, 0) * Translate(0, -0.01, 0) * Translate(0, 0, -0.14);
	xetai_canhxephai();
	model_truck = RotateY(a) * Translate(len, 0, 0) * Translate(0, -0.01, 0) * Translate(0, 0, 0.14);
	xetai_canhxetrai();
	model_truck = RotateY(a) * Translate(len, 0, 0) * Translate(-0.095, -0.01, 0);
	xetai_mattren();
	model_truck = RotateY(a) * Translate(len, 0, 0) * Translate(0.15, 0.065, 0);
	xetai_matduoi();
	model_truck = RotateY(a) * Translate(len, 0, 0) * Translate(0.3 + 0.15, 0.065, 0);
	xetai_cuasau();
	model_truck = RotateY(a) * Translate(len, 0, 0);
	xetai_banhxe();
	model_truck = RotateY(a) * Translate(len, 0, 0);
	//	xetai_denxe();
		//model_truck = RotateY(a) * Translate(xx, yy, zz);
		//xetai_taycam();
...
//MÔ HÌNH XE CỦA NGỌC
mat4 model_xeNgoc;
mat4 model_xeNgoc_ongNhom;
mat4 model_xeNgoc_BanhXe;
mat4 model_khungBanh;
GLfloat quayOngNhomUpperY, quayOngNhomUpperZ;
GLfloat dichChuyenOngNhomLower;
GLfloat xeNgoc_dichChuyen;
GLfloat xeNgoc_quayBanhXe;
GLfloat xeNgoc_quayY, xeNgoc_quayX;
GLfloat xeNgoc_quayCuaPhaiTruoc, xeNgoc_quayCuaPhaiSau, xeNgoc_quayCuaTraiTruoc, xeNgoc_quayCuaTraiSau;
GLfloat xeNgoc_quayTayCamTraiTruoc, xeNgoc_quayTayCamTraiSau;
GLfloat xeNgoc_quayTayCamPhaiTruoc, xeNgoc_quayTayCamPhaiSau;
//ong Nhom cua Ngoc

void ongNhom_base() {

	mat4 instance = Translate(0, 0.025, 0) * Scale(0.04, 0.05, 0.04);
	VeHinhHop(instance, model_xeNgoc * model_xeNgoc_ongNhom);
}
void ongNhom_upper() {
	mat4 instance = Translate(0, 0.2, 0) * Scale(0.03, 0.4, 0.03);
	VeHinhHop(instance, model_xeNgoc * model_xeNgoc_ongNhom);
}
void ongNhom_lower() {
	mat4 instance = Translate(0, 0.05, 0) * Scale(0.02, 0.15, 0.02);
	VeHinhHop(instance, model_xeNgoc * model_xeNgoc_ongNhom);
}
void ongNhom() {
	setMau(color4(0.2, 0.2, 0.2, 1));
	model_xeNgoc_ongNhom = Translate(0.05, 0.35, 0);
	ongNhom_base();
	model_xeNgoc_ongNhom *= Translate(0, 0.04, 0) * RotateY(quayOngNhomUpperY) * RotateZ(quayOngNhomUpperZ);
	ongNhom_upper();
	model_xeNgoc_ongNhom *= Translate(0, dichChuyenOngNhomLower, 0) * Translate(0, 0.4, 0);
	setMau(color4(0.35, 0.35, 0.35, 1));
	ongNhom_lower();
}

//banh Xe
void banh(float x, float y, float z, float m, float n, float p) {
	mat4 instance = Translate(m, n, p) * RotateZ(xeNgoc_quayBanhXe) * RotateX(90) * Scale(x, y, z);
	VeHinhTru(instance, model_xeNgoc);
}
void xeNgoc_boBanh() {
	setMau(color4(0.1, 0.1, 0.1, 1.0));
	banh(0.1, 0.05, 0.1, 0.35, 0, 0.25);
	banh(0.1, 0.05, 0.1, 0.35, 0, -0.25);
	banh(0.1, 0.05, 0.1, -0.4, 0, 0.25);
	banh(0.1, 0.05, 0.1, -0.4, 0, -0.25);
}


void boLanHoa_0(float x, float y, float z) {
	mat4 instance_lanhoa = Translate(0, 0.025, 0) * Scale(0.01, 0.05, 0.12);
	VeHinhHop(instance_lanhoa, model_xeNgoc * Translate(x, y, z) * RotateZ(xeNgoc_quayBanhXe));
	mat4 ins = RotateZ(120) * instance_lanhoa;
	VeHinhHop(ins, model_xeNgoc * Translate(x, y, z) * RotateZ(xeNgoc_quayBanhXe));
	mat4 ins1 = RotateZ(240) * instance_lanhoa;
	VeHinhHop(ins1, model_xeNgoc * Translate(x, y, z) * RotateZ(xeNgoc_quayBanhXe));
}

void boLanHoa() {
	setMau(color4(1, 0.9, 0, 1.0));
	boLanHoa_0(0.35, 0, 0.25);
	boLanHoa_0(0.35, 0, -0.25);
	boLanHoa_0(-0.4, 0, 0.25);
	boLanHoa_0(-0.4, 0, -0.25);

}
//vành xe
void xeNgoc_khungBanh(float x, float y, float z) {

	mat4 instance1 = Translate(x, y, z) * Translate(0, 0.03, 0) * RotateZ(0) * Scale(0.16, 0.025, 0.15);
	mat4 instance2 = Translate(x, y, z) * Translate(-0.1, 0.01, 0) * RotateZ(40) * Scale(0.08, 0.02, 0.15);
	mat4 instance3 = Translate(x, y, z) * Translate(0.1, 0.01, 0) * RotateZ(-40) * Scale(0.08, 0.02, 0.15);
	VeHinhHop(instance1, model_xeNgoc);
	VeHinhHop(instance2, model_xeNgoc);
	VeHinhHop(instance3, model_xeNgoc);
}

void xeNgoc_vanhXe() {
	setMau(color4(0.1, 0.1, 0.1, 1));
	xeNgoc_khungBanh(-0.4, 0.1, -0.2 - 0.15 / 2);
	xeNgoc_khungBanh(0.35, 0.1, 0.2 + 0.15 / 2);
	xeNgoc_khungBanh(-0.4, 0.1, 0.2 + 0.15 / 2);
	xeNgoc_khungBanh(0.35, 0.1, -0.2 - 0.15 / 2);
}
//Thân xe
////khung xe
mat4 model_khung;
void xeNgoc_khung(float x, float y, float z, float m, float n, float p, float f) {
	mat4 instance = Translate(x, y, z) * RotateZ(f) * Scale(m, n, p);
	VeHinhHop(instance, model_xeNgoc * model_khung);
}
void dauXe() {
	model_khung = Translate(-0.54, 0, -0.2);
	xeNgoc_khung(0, 0.05, 0.02, 0.55, 0.1, 0.04, 0);//dưới bên phải
	xeNgoc_khung(0.075, 0.1 + 0.08 / 2, 0.02, 0.4, 0.08, 0.04, 0);//trên bên phải
	//setMau(color4(0.1, 0.1, 0.1, 1));
	xeNgoc_khung(-0.55 / 2 - 0.02, 0.05, 0.2, 0.04, 0.1, 0.4, 0);//dưới trước dưới
	//setMau(color4(1, 0.9, 0, 1.0));
	xeNgoc_khung(-0.2, 0.12, 0.2, 0.2, 0.12, 0.4, 40);//dưới trước trên
	xeNgoc_khung(0, 0.05, 0.4 - 0.02, 0.55, 0.1, 0.04, 0);//dưới bên trái
	xeNgoc_khung(0.075, 0.1 + 0.08 / 2, 0.4 - 0.02, 0.4, 0.08, 0.04, 0);//trên bên trái
	xeNgoc_khung(0.06, 0.05 / 2 + 0.08 + 0.1, 0.2, 0.44, 0.05, 0.4, 0);
}
void muiXe() {
	model_khung = Translate(-0.05, 0.26, 0);
	xeNgoc_khung(0, 0.08, 0, 0.28, 0.04, 0.4, 0);
	setMau(color4(0.7, 0.8, 0.8, 0.5));
	xeNgoc_khung(-0.2, 0.02, 0, 0.2, 0.04, 0.4, 40);
	setMau(color4(1, 0.9, 0, 1.0));
	xeNgoc_khung(0.2, 0.02, 0, 0.2, 0.04, 0.4, -40);
...
void duoiXe() {
	model_khung = Translate(0.42, 0, 0);
	xeNgoc_khung(0, 0.15, 0.2 - 0.02, 0.4, 0.22, 0.04, 0);
	xeNgoc_khung(0, 0.15, -0.2 + 0.02, 0.4, 0.22, 0.04, 0);

	xeNgoc_khung(-0.2 - 0.02, 0.15, 0, 0.04, 0.22, 0.4, 0);
	xeNgoc_khung(0.2 + 0.01, 0.13, 0, 0.02, 0.26, 0.4, 0);
}

void cuaXe() {
	//cua truoc ben phai
	setMau(color4(1, 0.9, 0, 1.0));
	model_khung = Translate(-0.175, 0, -0.2 + 0.01) * Translate(-0.08, 0, 0) * RotateY(xeNgoc_quayCuaPhaiTruoc) * Translate(0.08, 0, 0);
	xeNgoc_khung(0, 0.18 / 2 + 0.04, 0, 0.14, 0.18, 0.02, 0);
	xeNgoc_khung(0.14 / 2 + 0.01, 0.28 / 2 + 0.04, 0, 0.02, 0.28, 0.02, 0);
	xeNgoc_khung(-0.01 - 0.14 / 2, 0.2 / 2 + 0.04, 0, 0.02, 0.2, 0.02, 0);
	xeNgoc_khung(0.033, 0.04 + 0.28 - 0.01, 0, 0.07, 0.02, 0.02, 0);
	xeNgoc_khung(-0.033, 0.03 + 0.04 + 0.2, 0, 0.1, 0.02, 0.02, 40);
	//tay cam;
	setMau(color4(0.35, 0.35, 0.35, 1));
	model_khung *= Translate(0.02, 0.04 + 0.15, -0.02) * Translate(-0.02, 0, 0) * RotateZ(xeNgoc_quayTayCamPhaiTruoc) * Translate(0.02, 0, 0);
	mat4 ins = Scale(0.04, 0.01, 0.02);
	VeHinhHop(ins, model_xeNgoc * model_khung);
	//cua truoc ben trai
	setMau(color4(1, 0.9, 0, 1.0));
	model_khung = Translate(-0.175, 0, 0.2 - 0.01) * Translate(-0.08, 0, 0) * RotateY(xeNgoc_quayCuaTraiTruoc) * Translate(0.08, 0, 0);
	xeNgoc_khung(0, 0.18 / 2 + 0.04, 0, 0.14, 0.18, 0.02, 0);
	xeNgoc_khung(0.14 / 2 + 0.01, 0.28 / 2 + 0.04, 0, 0.02, 0.28, 0.02, 0);
	xeNgoc_khung(-0.01 - 0.14 / 2, 0.2 / 2 + 0.04, 0, 0.02, 0.2, 0.02, 0);
	xeNgoc_khung(0.033, 0.04 + 0.28 - 0.01, 0, 0.07, 0.02, 0.02, 0);
	...
	//tay cam;
	setMau(color4(0.35, 0.35, 0.35, 1));
	model_khung *= Translate(0.02, 0.04 + 0.15, 0.02) * Translate(-0.02, 0, 0) * RotateZ(xeNgoc_quayTayCamTraiTruoc) * Translate(0.02, 0, 0);
	mat4 ins1 = Scale(0.04, 0.01, 0.02);
	VeHinhHop(ins1, model_xeNgoc * model_khung);
	//cua sau ben phai 
	setMau(color4(1, 0.9, 0, 1.0));
	model_khung = Translate(0.06, 0, -0.2 + 0.01) * Translate(-0.1, 0, 0) * RotateY(xeNgoc_quayCuaPhaiSau) * Translate(0.1, 0, 0);
	xeNgoc_khung(-0.2 / 2, 0.28 / 2 + 0.04, 0, 0.02, 0.28, 0.02, 0);
	xeNgoc_khung(0, 0.18 / 2 + 0.04, 0, 0.2, 0.18, 0.02, 0);
	xeNgoc_khung(0.18 / 2 + 0.02, 0.2 / 2 + 0.04, 0, 0.02, 0.2, 0.02, 0);//cot ngang
	xeNgoc_khung(-0.04, 0.04 + 0.28 - 0.01, 0, 0.1, 0.02, 0.02, 0);//khung ngang
	xeNgoc_khung(0.06, 0.03 + 0.04 + 0.2, 0, 0.13, 0.02, 0.02, -40);
	//taycam
	setMau(color4(0.35, 0.35, 0.35, 1));
	model_khung *= Translate(0.02, 0.04 + 0.15, -0.02) * Translate(-0.02, 0, 0) * RotateZ(xeNgoc_quayTayCamPhaiSau) * Translate(0.02, 0, 0);
	mat4 ins2 = Scale(0.04, 0.01, 0.02);
	VeHinhHop(ins2, model_xeNgoc * model_khung);

	//cua sau ben trai
	setMau(color4(1, 0.9, 0, 1.0));
	model_khung = Translate(0.06, 0, 0.2 - 0.01) * Translate(-0.1, 0, 0) * RotateY(xeNgoc_quayCuaTraiSau) * Translate(0.1, 0, 0);
	xeNgoc_khung(-0.2 / 2, 0.28 / 2 + 0.04, 0, 0.02, 0.28, 0.02, 0);
	xeNgoc_khung(0, 0.18 / 2 + 0.04, 0, 0.2, 0.18, 0.02, 0);
	xeNgoc_khung(0.18 / 2 + 0.02, 0.2 / 2 + 0.04, 0, 0.02, 0.2, 0.02, 0);//cot ngang
	xeNgoc_khung(-0.04, 0.04 + 0.28 - 0.01, 0, 0.1, 0.02, 0.02, 0);//khung ngang
	...
	//taycam
	setMau(color4(0.35, 0.35, 0.35, 1));
	model_khung *= Translate(0.02, 0.04 + 0.15, 0.02) * Translate(-0.02, 0, 0) * RotateZ(xeNgoc_quayTayCamTraiSau) * Translate(0.02, 0, 0);
	mat4 ins3 = Scale(0.04, 0.01, 0.02);
	VeHinhHop(ins3, model_xeNgoc * model_khung);
}
color4 denXeNgoc;
void xeNgoc_den() {
	setMau(denXeNgoc);
	mat4 m = Scale(0.01, 0.01, 0.1);
	VeHinhHop(m, model_xeNgoc * Translate(-0.8, 0.145, 0.12) * Translate(0, 0.01 * 0.5, 0) * RotateZ(-40));

	VeHinhHop(m, model_xeNgoc * Translate(-0.8, 0.145, -0.12) * Translate(0, 0.02 * 0.5, 0) * RotateZ(-40));
}
void xeNgoc_thanXe() {
	//xeNgoc_dauXe();
	setMau(color4(1, 0.9, 0, 1.0));
	...
	setMau(color4(0.1, 0.1, 0.1, 1));
	mat4 lotDuoi = Translate(-0.1 + 0.02, 0.02, 0) * Scale(1.4, 0.04, 0.32);
	VeHinhHop(lotDuoi, model_xeNgoc);
	mat4 khungphai = Translate(-0.068, 0.28 / 2 + 0.04, -0.2 + 0.01) * Scale(0.03, 0.28, 0.02);
	VeHinhHop(khungphai, model_xeNgoc);
	mat4 khungtrai = Translate(-0.068, 0.28 / 2 + 0.04, 0.2 - 0.01) * Scale(0.03, 0.28, 0.02);
	VeHinhHop(khungtrai, model_xeNgoc);
	xeNgoc_den();
}
...
void Xe_Ngoc() {
	...
	model_xeNgoc = Translate(xeNgoc_dichChuyen, 0, 0) * RotateY(xeNgoc_quayY) * RotateX(xeNgoc_quayX);
	xeNgoc_thanXe();
	xeNgoc_vanhXe();
	xeNgoc_boBanh();
	boLanHoa();
...
void display(void)
{
	...
	model_may = Translate(-14 + xmay, 0, 7) * Scale(0.8, 1, 0.8);
	may();
	model_may = Translate(8 + xmay, 0, 5) * Scale(1.1, 1, 1.1);
	may();
	model_may = Translate(-4 + xmay, 0, 5) * Scale(1.1, 1, 1.1);
	may();
	model_may = Translate(8 + xmay, 0, 5) * Scale(1.1, 1, 1.1);
	may();
	model_may = Translate(-4 + xmay, 0, 0) * Scale(0.6, 1, 0.6);
	may();
	mat4 temp_model = RotateY(quayAllY);
	model = temp_model * Translate(-2.1, 0, -0.5) * RotateY(55) * Scale(1.35, 1.35, 1.35);
	XeTang();
	model = temp_model * RotateY(55) * Translate(0.5, 0, -0.8);
	XeTenLuaSieuVip();
	model = temp_model * Translate(-0.2, 0, -5) * Scale(0.3, 0.3, 0.3);
	RoBot_Nghia();
	sanNha();
	model_xe_phao = Translate(1.1, 0.47,-0.2) * RotateX(theta[0]) * RotateY(theta[1] - 105) * RotateZ(theta[2]) * Translate(theta[3], 0, 0) * Scale(1.3, 1.3, 1.3);
	xephao_MNghia();
	model = Translate(0, 3, 0) * RotateY(18 * 5) * Scale(2, 2, 2);
	maybay();
	model = temp_model * Translate(4, 0, -0.3) * Scale(0.7, 0.7, 0.7);
	xeV();
	model = temp_model * Translate(2.6, 0.55, -0.2) * RotateY(-18 * 6) * Scale(2.5, 2.5, 2.5);
	Truck();
	model = temp_model * Translate(-3.8, 0.2, -0.2) * RotateY(-120) * Scale(1.8, 1.8, 1.8);
	Xe_Ngoc();
	/*sanNha();
...
void Idle_Nghia() {
	xmay += 0.01;
	if (xmay > 8) xmay = -8;
	if (shootRobot == true && check_sung == false) {
		xxSungRobot -= 0.003;
		heSo += 0.2f;
...
void keyboard(unsigned char key, int x, int y)
{
	...
	if (NguyenThiLinhNgan_control == true) {
		...
		case 'n':
			a = a - 10;
			xetai_quaybanhxe = xetai_quaybanhxe + 20;
			glutPostRedisplay();
			break;
		case 'N':
			a = a + 10;
			...
			break;
		case 'b':
			quaycanhphai = quaycanhphai + 20;
			if (quaycanhphai >= 90) quaycanhphai = 90;
			glutPostRedisplay();
			break;
		case 'B':
			quaycanhphai = quaycanhphai - 20;
			if (quaycanhphai <= 0) quaycanhphai = 0;
			glutPostRedisplay();
			break;
		case 'c':
			quaycanhtrai = quaycanhtrai - 20;
			if (quaycanhtrai <= -90) quaycanhtrai = -90;
			glutPostRedisplay();
			break;
		case 'C':
			quaycanhtrai = quaycanhtrai + 20;
			if (quaycanhtrai >= 0) quaycanhtrai = 0;
			glutPostRedisplay();
			break;
		case 'r':
			quaycuasau1 = quaycuasau1 - 20;
			if (quaycuasau1 <= -210) quaycuasau1 = -210;
			glutPostRedisplay();
			break;
		case 'R':
			quaycuasau1 = quaycuasau1 + 20;
			if (quaycuasau1 >= 0) quaycuasau1 = 0;
			glutPostRedisplay();
			break;
		case 'u':
			quaycuasau2 = quaycuasau2 + 20;
			if (quaycuasau2 >= 210) quaycuasau2 = 210;
			glutPostRedisplay();
			break;
		case 'U':
			quaycuasau2 = quaycuasau2 - 20;
			if (quaycuasau2 <= 0) quaycuasau2 = 0;
			glutPostRedisplay();
			break;
		case 'l':
			len = len - 0.1;
	...
	if (NguyenThiBichNgoc_control == true) {
		...
		case 'x':
			...
			//lệnh mở, đóng cửa
		case 'c':
			xeNgoc_quayTayCamPhaiTruoc -= 2;
			while (xeNgoc_quayTayCamPhaiTruoc < -60) {
				xeNgoc_quayTayCamPhaiTruoc = -60;
				xeNgoc_quayCuaPhaiTruoc += 5;
				if (xeNgoc_quayCuaPhaiTruoc > 70) xeNgoc_quayCuaPhaiTruoc = 70;
			}
			glutPostRedisplay();
			break;
		...
		case 'C':
			xeNgoc_quayCuaPhaiTruoc -= 5;
			if (xeNgoc_quayCuaPhaiTruoc < 0) {
				xeNgoc_quayCuaPhaiTruoc = 0;
				xeNgoc_quayTayCamPhaiTruoc = 0;
			}
			glutPostRedisplay();
			break;
		case 'v':
			xeNgoc_quayTayCamTraiTruoc -= 2;
			while (xeNgoc_quayTayCamTraiTruoc < -60) {
				xeNgoc_quayTayCamTraiTruoc = -60;
				xeNgoc_quayCuaTraiTruoc -= 5;
				if (xeNgoc_quayCuaTraiTruoc < -70) xeNgoc_quayCuaTraiTruoc = -70;
			}
		...
		case 'V':
			xeNgoc_quayCuaTraiTruoc += 5;
			if (xeNgoc_quayCuaTraiTruoc > 0) {
				xeNgoc_quayCuaTraiTruoc = 0;
				xeNgoc_quayTayCamTraiTruoc = 0;
			}
			glutPostRedisplay();
			break;
		case 'b':
			xeNgoc_quayTayCamPhaiSau -= 2;
			while (xeNgoc_quayTayCamPhaiSau < -60) {
				xeNgoc_quayTayCamPhaiSau = -60;
				xeNgoc_quayCuaPhaiSau += 5;
				if (xeNgoc_quayCuaPhaiSau > 70) xeNgoc_quayCuaPhaiSau = 70;
			}
			glutPostRedisplay();
			break;
		...
		case 'B':
			xeNgoc_quayCuaPhaiSau -= 5;
			if (xeNgoc_quayCuaPhaiSau < 0) {
				xeNgoc_quayCuaPhaiSau = 0;
				xeNgoc_quayTayCamPhaiSau = 0;
			}
			glutPostRedisplay();
			break;
		case 'n':
			xeNgoc_quayTayCamTraiSau -= 2;
			while (xeNgoc_quayTayCamTraiSau < -60) {
				xeNgoc_quayTayCamTraiSau = -60;
				xeNgoc_quayCuaTraiSau -= 5;
				if (xeNgoc_quayCuaTraiSau < -70) xeNgoc_quayCuaTraiSau = -70;
			}
		...
		case 'N':
			xeNgoc_quayCuaTraiSau += 5;
			if (xeNgoc_quayCuaTraiSau > 0) {
				xeNgoc_quayCuaTraiSau = 0;
				xeNgoc_quayTayCamTraiSau = 0;
			}
			glutPostRedisplay();
			break;
			//lệnh bật đèn
		case 'i':
			denXeNgoc = color4(1, 1, 1, 1);
			glutPostRedisplay();
			break;

		case 'I':
			denXeNgoc = color4(0.1, 0.1, 0.1, 1);
			glutPostRedisplay();
			break;
